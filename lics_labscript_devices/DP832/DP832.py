...
class DP832():
    ...
    def set_voltage(self, channel, val):
        if channel in [1,2] and val <=30: #check that the value doesn't exceed the limits defined by the device
            self._set(channel, "VOLT", val)
        elif channel==3 and val <=5:
            self._set(channel, "VOLT", val)
        elif channel in [1,2] and val > 30:
            print("Error: Input for channel" + str(channel) + ' must be less than 30V')
        elif channel ==3 and val > 5:
            print("Error: Input for channel " + str(channel) + ' must be less than 5V' )
    
    def set_current(self, channel, val):
        if val <= 3:
            self._set(channel, "CURR", val)
        else:
            print("Error: Input must be less than 3A")
...
class DP832Service(rpyc.Service):
    def __init__(self):
        ...
        self.rm = pyvisa.ResourceManager() # Initialize VISA manager
    ...
    def exposed_connect_device(self, device):
        self.devices[device][0].connect()

    def exposed_close_device(self, device):
        self.devices[device][0].close()
